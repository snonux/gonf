...
// RedactSecrets (metadata only for strong secrets). This is the one caller
// of redactOpStrings (task kf2 split it from scanOpStrings, the read-only
// walker every other caller uses instead): it legitimately rewrites
// strings, and it does so on out, its own copyOp deep copy, never on the
// caller's op — so writing the rewritten Op.Payload back is safe here even
// though the same write-back would race a concurrent reader on a shared op
// (see scanOp and opDisplayName, task 0f2).
