...
// recorded by older binaries (v1–v6) must still decode after a schema bump.
...
// up-front, and newer binaries must keep applying v1–v6 plans.
func TestDecodePlanAcceptsOlderVersions(t *testing.T) {
	t.Parallel()
	for _, version := range []int{1, 2, 3, 4, 5, 6, CurrentVersion} {
