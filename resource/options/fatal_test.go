...
var fatalCases = []struct {
	...
	{"IfChanged without the DependsOn fallback", func() { IfChanged.Apply(&watchOnly{}) }, "*options.watchOnly does not support IfChanged"},
	{"WithWatch without the DependsOn fallback", func() { WithWatch("File[a]").Apply(&watchOnly{}) }, "*options.watchOnly does not support WithWatch"},
	{"empty WithWatch without the DependsOn fallback", func() { WithWatch().Apply(&watchOnly{}) }, "*options.watchOnly does not support WithWatch"},
