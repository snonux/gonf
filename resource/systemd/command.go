...
// build their convergence steps from Command and run them through the shared
// runner resource.Converge, and DaemonReload renders its log text with
// Describe, so all three run and describe systemctl identically.
...
var _ resource.Action = Command(nil)

// Command is the resource.Action for one systemctl invocation: its argument
// vector after "systemctl" (build it with Args to get --user handling).
type Command []string
...
// Do runs systemctl with the command's arguments; the errors are Run's.
