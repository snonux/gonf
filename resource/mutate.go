...
// Individual resource kinds wrap their mutation in Mutate instead of
// hand-rolling their own "if DryRun() { note; log; skip }" branch, so a NEW
// resource kind (or an edit to an existing one) cannot forget the dry-run
// gate on a mutation: Mutate provides it centrally, and the resource kinds
// that still hand-roll their own check are covered by
// api's TestDryRunFitness regression test either way.
//
...
// dry-run log line ("dry-run: would " + desc). Callers keep whatever success
// logging they already had inside fn.
//
...
func Mutate(id, desc string, fn func() error) error {
	if DryRun() {
		...
		logger.Info("dry-run: would %s", desc)
		return nil
	}
	if err := fn(); err != nil {
		return err
