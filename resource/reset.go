...
// clears the declaration errors (internal/declerr) that DSL misuse reported,
// which every resource constructor feeds. The individual functions it composes
// (ResetRepository, ResetReport, SetDryRun) stay available for callers that
// reset exactly one piece, so existing tests keep working.
//
