...
| `inetd` (flags append; login.conf.d/inetd; inetd.conf restart-on-change; service) | `frontends_inetd`: `File(WithLine)` + 2× `InstallFile` + `cap_mkdb` + `Service("inetd", WithRestart, OnChange(...))` | **Consumer** — task s42; local plan verified, live rollout remains an explicit operator action |
| `relayd` (flags append; login.conf.d/daemon + `cap_mkdb` on change; relayd.conf 0600 restart-on-change; service; daily.local append) | `frontends_relayd` controller-renders, stages and validates `relayd.conf`, rebuilds login DB on change, then change-gates the restart | **Consumer** — task s42; local plan verified, live rollout remains an explicit operator action |
...
| `nsd_failover` (script + root crontab via run) | `frontends_dns_failover`: installs the script, removes only legacy unmarked DNS-failover lines outside its own marker block, then creates marker-managed `Cron("frontend-nsd-failover", WithCommand("-ns /usr/local/bin/dns-failover.ksh"), WithMinute("*"))` | **Consumer** — task t42; local plan and duplicate-legacy-line cleanup fixture verified. Live rollout remains an explicit operator action |
...
| `gogios` (pkg ×2; adduser `_gogios`; dirs; gogios.json template over 3 arrays; `check_shuriken_age` sourced from `~/git/shuriken.sh`; `_gogios` crontab from template; rc.local appends) | `frontends_gogios` creates `_gogios`, converges runtime/status directories, Go-renders `gogios.json`, installs the external plugin, removes only the three legacy unmarked Gogios cron commands before managing them, and preserves boot-time runtime-directory setup | **Consumer** — task u42; Garage and virtual-hosted bucket checks retain task 642's expected HTTP 403 result |
