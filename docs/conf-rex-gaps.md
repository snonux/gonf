...
Refreshed 2026-09-24 against **gonf v0.18.0**; this release uses plan schema 24.
...
- **One consumer module per repository**, depending on `github.com/snonux/gonf`
  (conf pins v0.17.0, dotfiles v0.16.1; any later consumer upgrade is a
...
Status against every conf Rex primitive in v0.18.0 (plan schema 24):

| Conf Rex capability | gonf v0.18.0 | Status |
|---------------------|--------------|--------|
...
what reaches the plan. conf uses this (conf task ze2 and its follow-up): its
`main` installs `secret.NewSnapshot(secret.NewFallback(vault,
secret.FileProvider{}))`, where `vault` is the `secret/foostore` provider
mapping four references (the NSD TSIG key, the Garage RPC secret and the two
frontends' goprecords upload tokens) to KeePass entries; every other
reference still reads `gonf/secrets/` (see secrets.md, "Staged cutover").
...
- Every capability row names the gonf API that exists today (verified
  against v0.18.0, plan schema 24) — no "fleet needs transport" or
