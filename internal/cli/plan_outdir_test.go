...
func TestCLIWithSymlinkedTempDir(t *testing.T) {
	...
	*ran = false
	// gonf -n no longer leaves resource's process-wide dry-run flag set
	// behind: CLI() restores it on return (scopeDryRun, task vg2), so this
	// test needs no reset of its own.
